...
class ByTextOrImageRemover(RemoverBase):
    ...
    def _compare_two_files(self, file_infos : list[FileInfo], fi1_idx : int, compared_hash_type : HashType) -> None:
        ...
        if text_sim_score >= MIN_SIMILARITY and compared_hash_type == HashType.TEXT:
            self._manage_remove((text_sim_score, img_sim_score), file_infos, fi1_idx, fi2_idx)
        elif img_sim_score >= MIN_SIMILARITY and compared_hash_type == HashType.IMAGE:
    ...
    def _manage_remove(self, sim_score : tuple[float], file_infos : list[FileInfo], fi1_idx : int, fi2_idx : int) -> None:
        ...
        if (main_score >= AUTO_REMOVE_SIMILARITY and second_score >= AUTO_REMOVE_SIMILARITY
